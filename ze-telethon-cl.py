...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        buttons = [
            ...
            [Button.inline('• اوامر اخرى • ', 'other'),Button.inline('< ABOUT >', 'ze')],
        ]
...
async def listb(event):
    buttons = [
        ...
        [Button.inline('⋆ تعيين البوت المطلوب ⋆', 'selebot'), Button.inline('⋆ بدء التجميع ⋆', 'oncollect'), Button.inline('⋆ ايقاف التجميع ⋆', 'offcollect')],
        [Button.inline('⋆ تحويل النقاط ⋆', 'transferpt'),Button.inline('⋆ عدد النقاط ⋆', 'infopt'),Button.inline('⋆ الهدية اليومية ⋆', 'gifkeko')],
        [Button.inline('⋆ حضر البوت ⋆', 'banbot'), Button.inline('⋆ فك حضر البوت ⋆', 'unbanbot'),Button.inline('⋆ مغادرة القنوات ⋆', 'leavekeko')],
        [Button.inline('⋆ تجميع Keko Api ⋆', 'collectapiko'), Button.inline('⋆ ايقاف تجميع Api KO ⋆', 'spkoai')],    
        [Button.inline('• اوامر اخرى 🔽 • ', 'list3'),Button.inline('⋆ بوت دعمكم ⋆', 'dambot')]
    ]
...
∘ مطور ومبرمج البوت مودي الهيبه - @D_S_I
...
async def callback(event):
    await event.edit("**• قـم بأرسال المطاليب التالية :**", buttons=[Button.inline("• رجــوع • ", 'list1')])
    async with bot.conversation(event.sender_id) as conv:
        await conv.send_message("**✪ قـم بإرسال رابط دعوة المجلد الذي يحتوي على القنوات**")
        invitation_link = (await conv.get_response()).text.strip()

        try:
            # Extract the hash from the invitation link
            match = re.search(r'https://t.me/addlist/(\S+)', invitation_link)
            if match:
                folder_hash = match.group(1)

                # Import the chat invite to join the folder
                result = await bot(ImportChatInviteRequest(folder_hash))

                # Check if the chat was successfully imported
                if result:
                    await conv.send_message("**✪ تم الانضمام إلى القنوات في المجلد باستخدام رابط الدعوة.**")
                    await conv.send_message("**✪ قـم بأرسال يوزر القناة المراد الانضمام بها**")
                    bot_username = (await conv.get_response()).text

                    # Notify stored users about the joined channel
                    for user_id in stored_users:
                        try:
                            await bot.send_message(user_id, f"/jn {bot_username}")
                        except Exception as e:
                            # Handle exceptions as needed
                            pass
                else:
                    await conv.send_message("**❌ خطأ: فشل الانضمام إلى القنوات في المجلد.**")
            else:
                await conv.send_message("**❌ خطأ: رابط الدعوة غير صالح.**")
        except Exception as e:
            print(f"Error joining chat folder: {e}")
