...
bad_guys_file = 'database/bad_guys.json'
force_file = 'database/force.json'
used_gift_links_file = 'database/used_gift_links.json'

# Check if database directory exists, create if not
if not os.path.isdir('database'):
    os.mkdir('database')

# Replace your database operations with JSON file operations
acc = read_from_json(accounts_file)
if not acc:
    write_to_json(accounts_file, [])

bad_guys = read_from_json(bad_guys_file)
if not bad_guys:
    write_to_json(bad_guys_file, [])

force = read_from_json(force_file)
if not force:
    write_to_json(force_file, [])
