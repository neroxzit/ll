...
async def get_urls(acc):
	urls = ""
	msgs = acc.get_chat_history(777000, limit=None)
	...
	return urls
#——————–——–———————————––#
async def join_channel(link_or_username):
...
async def main_menu():
	...
	[InlineKeyboardButton("انضمام للقناة", callback_data="join_channel")]
	]
...
#——————–——–———————————––#
		...
		processing = True
		try:
			channel_ask = await msg.chat.ask(
				"ارسل رابط أو معرف القناة للانضمام:",
				filters=filters.text
				)
			channel = channel_ask.text
			await join_channel(channel)
			await app.send_message(msg.chat.id, f"تم الانضمام إلى القناة: {channel}")
		except Exception as e:
			await app.send_message(msg.chat.id, f"حدث خطأ أثناء الانضمام إلى القناة: {e}")
		finally:
			processing = False
#——————–——–———————————––#
