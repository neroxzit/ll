...
from messagesBots import KitListt
from messagesBots import KitLisst
from messagesBots import KitListbot
import random
...
def gamesBgr(message: Message):
    ...
    if (
        ...
        if show_group(chat_id)["games"]:
            abotmsg = random.choice(KitListbot)
            bot.send_message(
                chat_id,
                f"<b> {abotmsg}</b>",
                reply_to_message_id=message.id,
